...
describe("einaPage", () => {
  ...
  it("computed details", () => {
    ...
    expect(page.details).toBe(
      `Mostrando ${data.currentrows} de ${data.rowcount} filas en la página ${data.currentpage} de ${data.pagecount}.`
    );
  });

  it("computed shouldSeeMore", () => {
    const einaPage = Vue.extend(EinaPage);
    const page = new einaPage({
      propsData: {
        currentpage: 4,
        pagecount: 1255,
      },
  ...
  it("test styles", (done) => {
    ...
    Vue.nextTick(() => {
      expect(
        page.$el.getElementsByClassName("overlayclass").length
      ).toBeTruthy();
      expect(
        page.$el.getElementsByClassName("detailsclass").length
      ).toBeTruthy();
      expect(page.$el.getElementsByClassName("detailsclass")[0].tagName).toBe(
        "LI"
      );

      expect(page.$el.getElementsByClassName("itemclass").length).toBeTruthy();
      expect(page.$el.getElementsByClassName("itemclass")[0].tagName).toBe(
        "LI"
      );

      expect(page.$el.getElementsByClassName("dataclass").length).toBeTruthy();
      expect(page.$el.getElementsByClassName("dataclass")[0].tagName).toBe(
        "SPAN"
      );
      done();
    });
