<template>
  <div class="_einapage">
    <ul :class="['_einapage__container', { [overlayclass]: overlayclass }]">
      <li :class="['_einapage__details', { [detailsclass]: detailsclass }]">
        <slot
          ...
          name="details"
        >
          <span :class="['_einapage__data', { [dataclass]: dataclass }]">
            {{ details }}
          </span>
        </slot>
      </li>
      <li :class="['_einapage__per-page', { [perpageclass]: perpageclass }]">
        <slot :perpage="$props.perpage" name="perPage">
          <select
            v-if="$props.perpage && $props.perpage.length"
            v-model="perPage"
            :class="{ [selectclass]: selectclass }"
          >
            <option v-for="s in $props.perpage" :key="'item-' + s" :value="s">
              {{ s }}
            </option>
          </select>
        </slot>
      ...
      >
        <a href="#" v-on:click.prevent="doPagination(1)">
          <slot name="firstArrow">
            <span
              :class="['_einapage__data', { [dataclass]: dataclass }]"
              aria-hidden="true"
              >&laquo;</span
            >
          </slot>
      ...
      <li
        ...
        :class="['_einapage__item', { [itemclass]: itemclass }]"
      >
      ...
      <li :class="['_einapage__item', { [itemclass]: itemclass }]">
